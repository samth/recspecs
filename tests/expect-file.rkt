#lang racket
(require rackunit rackunit/text-ui racket/file recspecs)
...
(define file-tests
  (test-suite
   ...
   (test-case "multi line"
     (expect-file (begin (displayln "a") (displayln "b"))
                  "data/multi.txt"))
   (test-case "strict mode"
     (expect-file (display "strict")
                  "data/strict.txt"
                  #:strict? #t))))
...
(define update-tests
  (test-suite
   ...
   (test-case "updates expectation file"
    ...
    (call-with-output-file tmp-out #:exists 'truncate/replace
      (lambda (out)
        (display (file->string src) out)))
     (define tmp-test (make-temporary-file "test~a.rkt"))
     (call-with-output-file tmp-test #:exists 'truncate/replace
       (lambda (out)
...
(module+ test
  (run-tests (test-suite "all" file-tests update-tests)))
